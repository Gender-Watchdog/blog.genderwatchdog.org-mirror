...
def detect_language(slug):
    """Detect language from slug suffix patterns"""
    ...
    # Korean
    if slug_lower.endswith('-ko'):
        return 'ko'
    
    # Simplified Chinese
    if slug_lower.endswith('-zh-cn') or slug_lower.endswith('-zh-ch') or 'hans' in slug_lower:
        return 'zh-cn'
    
    # Traditional Chinese
    if slug_lower.endswith('-zh-tw') or 'hant' in slug_lower:
        return 'zh-tw'
    
    # Check for other language patterns (Indonesian, Russian, Vietnamese, etc.)
    other_lang_patterns = ['-id', '-ru', '-vi', '-kk', '-mn', '-th', '-uz', '-fr']
